...
## 기술적 성취와 파이프라인
### feat: PostgreSQL 데이터 파이프라인 구축 및 DB 마이그레이션 자동화
- Flyway 도입: DB 형상 관리 도구(Flyway)를 연동하여 운영 환경의 스키마 버전 관리 자동화
- 데이터 아카이빙: 게임 완료/실패 시 Redis(휘발성) 데이터를 PostgreSQL(영구)로 전이하는 파이프라인 구현
- 트랜잭션 최적화: @Transactional 격리 수준에 따른 타이밍 이슈를 해결하기 위해 POST /save 응답 시 최신 데이터 반환 로직 적용
- 안정성 강화: Redis TTL 단축 및 신규 게임 생성 시 지연 삭제(Lazy Cleanup) 전략으로 자원 관리 최적화
## 트러블슈팅 및 비즈니스 로직 중심
### fix: 게임 종료 시 데이터 유실 방지 및 세션 관리 로직 개선
- Race Condition 해결: 삭제 전 스냅샷 확보 및 응답 DTO(PlaceResponse) 일괄 반환으로 최종 화면 렌더링 보장
- 로그인/로그아웃 UX 개선: 상태 기반 세션 관리로 전환하여 로그아웃 시 불필요한 API 요청(500 에러) 차단
- DB 스키마 설계: 개인 통계 기능을 위한 game_records 테이블 설계 및 하트(life) 수치 기록 추가
- 코드 리팩토링: 중복된 checkRecentGame 호출 제거 및 인터페이스 default 메서드를 활용한 유연한 저장소 구조 설계

### 참고: local solo playing 기준에서 했기 때문에 실제 배포 및 대규모 트래픽에 대한 정보는 없음 추후에 고도화 할 것

---

"초기 개발 단계에서는 빠른 기능 구현을 위해 IllegalArgumentException을 공통으로 사용했으나, HTTP 상태 코드의 명확한 전달(401 vs 404)이 프론트엔드와의 통신 규격에서 필수적임을 깨달았습니다. 이를 위해 @RestControllerAdvice를 도입하여 예외 처리를 중앙 집중화하고 유지보수성을 높였습니다." [2026-02-21]
...
- entire
  - 시간 제대로 연동하기(현재 네트워크 delay 시간 멈춤)
  - refresh 대비 url만 나타내기
  - refresh 및 각 상태 전환 시 상태 청소 필요
- multi
  - 멀티시 시간이 따로 흐름
  - 멀티시 채팅 연동
  - 멀티시 일시 정지 미지원 고려
